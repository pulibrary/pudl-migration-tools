...
(:~
 ...
 :
 : Sample input: Volume Folio 1, Leaf 5, Photograph j
 : Output: 1.5.j
 :
...
as xs:string
{
 let $hit := fn:analyze-string($string, ".*?Folio\s+([0-9]).*?Leaf\s+([0-9]+).*?Photograph\s+([a-zA-Z]+)")
...
let $ead_items := collection('/db/WC055')//ead:c[@level='file']
let $mods := collection('/db/WC055/wc055/1139550')//mods:mods
let $assets :=
 for $e in $ead_items
